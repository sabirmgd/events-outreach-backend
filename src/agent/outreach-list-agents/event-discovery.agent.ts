...
Extract comprehensive information about each event found.`;
    ...
    try {
      ...
      try {
        eventsWithDetails = await b.ExtractEvents(searchText);
        this.logger.log(`BAML returned: ${JSON.stringify(eventsWithDetails)}`);
      } catch (bamlError) {
        this.logger.error('BAML extraction failed:', bamlError);
        // Fallback to parsing the LangChain output directly
