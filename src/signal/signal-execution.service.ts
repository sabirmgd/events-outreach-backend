...
import {
  SignalExecution,
  ExecutionStatus,
} from './entities/signal-execution.entity';
...
export class SignalExecutionService {
  ...
  async executeSignal(
    ...
    this.runExecution(execution.id, signal).catch((error: Error) => {
      this.logger.error(`Execution ${execution.id} failed:`, error.stack);
    });

    return execution;
  }

  private emitSignalProgress(signalId: string, data: object) {
    ...
    try {
      ...
      switch (signal.type) {
        ...
        case SignalType.FUNDING:
          result = await this.executeFundingSignal();
          break;
      ...
      this.logger.log(
        `✅ Execution updated. Results saved: ${
          updatedExecution?.results ? 'YES' : 'NO'
        }`,
      );
  ...
  private async executeConferenceSignal(signal: Signal): Promise<any> {
    ...
    this.logger.log(
      `🎉 EVENTS DISCOVERED: Found ${eventDataArray.length} events!`,
    );
    eventDataArray.forEach((event, i) => {
      this.logger.log(
        `Event ${i + 1}: ${event.name} - ${event.date} - ${event.venue}`,
      );
    });
    ...
    const savedEvents = [];
    this.logger.log(
      `Processing ${eventDataArray.length} events from agent result`,
    );
      ...
      try {
        ...
        try {
          ...
          if (isNaN(eventDate.getTime())) {
            this.logger.warn(
              `Invalid date for event ${eventData.name}: ${eventData.date}, using current date`,
            );
            eventDate = new Date();
          }
        } catch (error) {
          this.logger.warn(
            `Failed to parse date for event ${eventData.name}: ${eventData.date}, using current date`,
          );
          eventDate = new Date();
        }
    ...
    this.logger.log(
      `✅ EVENTS SAVED: Successfully saved ${savedEvents.length} out of ${eventDataArray.length} events!`,
    );
    this.logger.log(
      `🔄 TRANSITIONING TO SPONSORS: Starting sponsor extraction for ${savedEvents.length} events...`,
    );
    ...
    this.logger.log(
      `🚀 PARALLEL SPONSOR EXTRACTION: Processing ${savedEvents.length} events in ${eventBatches.length} batches`,
    );
    ...
    for (const [batchIndex, eventBatch] of eventBatches.entries()) {
      this.logger.log(
        `Processing batch ${batchIndex + 1}/${
          eventBatches.length
        } with ${eventBatch.length} events`,
      );

      // Add delay between batches to avoid rate limiting (except for first batch)
      if (batchIndex > 0) {
        this.logger.log(
          `⏳ Waiting ${BATCH_DELAY}ms before next batch to avoid rate limits...`,
        );
      ...
      this.emitSignalProgress(signal.id, {
        ...
        status: 'running',
        eventsCompleted: savedEvents.length,
        message: `Extracting sponsors: Batch ${batchIndex + 1}/${
          eventBatches.length
        }...`,
        progress: Math.round(
          ((batchIndex * BATCH_SIZE) / savedEvents.length) * 100,
        ),
      });
      ...
      const sponsorPromises = eventBatch
        ...
        .map(async (event) => {
          ...
          if (existingSponsors.length > 0) {
            this.logger.log(
              `Event ${event.name} already has ${existingSponsors.length} sponsors, using cached data`,
            );
            ...
            for (const sponsor of existingSponsors) {
              ...
              if (
                !companyMap.has(normalizedName) &&
                (!normalizedWebsite ||
                  !companyWebsiteMap.has(normalizedWebsite))
              ) {
                // Reload company with hq_city if needed
                let companyWithLocation = company;
                if (!company.hq_city && company.id) {
                  companyWithLocation = await this.companyService.findOne(
                    company.id,
                    ['hq_city'],
                  );
                }
                ...
                const companyData = {
                  ...
                  location: companyWithLocation.hq_city
                    ? `${companyWithLocation.hq_city.name}, ${companyWithLocation.hq_city.country_code}`
                    : 'Unknown',
                };
                ...
                if (
                  !savedCompanies.some((c) => c.id === companyWithLocation.id)
                ) {
          ...
          while (retries < maxRetries) {
            try {
              const sponsorResult =
                await this.agentExecutionService.executeWithProgress(
                  'sponsor-extraction',
                  'extractSponsors',
                  {
              ...
              if (
                ...
                if (retries < maxRetries) {
                  this.logger.warn(
                    `Rate limit hit for ${event.name}, retrying in ${backoffDelay}ms (attempt ${retries}/${maxRetries})`,
                  );
              ...
              this.logger.error(
                `Failed to extract sponsors for ${event.name} after ${retries} retries:`,
                error,
              );
              return { event, sponsors: [] };
            }
        ...
        for (const sponsorData of sponsors) {
          ...
          try {
            ...
            if (!company && sponsorData.website) {
              ...
              company = await this.companyService.findByWebsite(
                sponsorData.website,
              );
            }
            ...
            const companyData = {
              ...
              location: company.hq_city
                ? `${company.hq_city.name}, ${company.hq_city.country_code}`
                : 'Unknown',
            };
            ...
            this.logger.error(
              `Failed to save company ${sponsorData.name}:`,
              error,
            );
          }
        }
    ...
    this.logger.log(
      `✅ SPONSORS SAVED: Successfully saved ${savedCompanies.length} companies from sponsors!`,
    );
    this.logger.log(
      `🔄 TRANSITIONING TO PEOPLE: Finding contacts at ${savedCompanies.length} companies...`,
    );
    ...
    const targetFunctions =
      ...
      configuration.target_functions.length > 0
        ? configuration.target_functions
        : ['CEO', 'CTO', 'CMO'];
    ...
    this.logger.log(
      `🚀 PARALLEL PEOPLE ENRICHMENT: Processing ${savedCompanies.length} companies in ${companyBatches.length} batches`,
    );

    for (const [batchIndex, companyBatch] of companyBatches.entries()) {
      this.logger.log(
        `Processing batch ${batchIndex + 1}/${
          companyBatches.length
        } with ${companyBatch.length} companies`,
      );

      // Add delay between batches to avoid rate limiting (except for first batch)
      if (batchIndex > 0) {
        this.logger.log(
          `⏳ Waiting ${PEOPLE_BATCH_DELAY}ms before next batch to avoid rate limits...`,
        );
      ...
      this.emitSignalProgress(signal.id, {
        ...
        status: 'running',
        companiesFound: savedCompanies.length,
        message: `Finding contacts: Batch ${batchIndex + 1}/${
          companyBatches.length
        }...`,
        progress: Math.round(
          ((batchIndex * PEOPLE_BATCH_SIZE) / savedCompanies.length) * 100,
        ),
      });
      ...
      const peoplePromises = companyBatch.map(async (company) => {
        ...
        if (existingContacts.length >= MAX_CONTACTS_PER_COMPANY) {
          this.logger.log(
            `Company ${company.name} already has ${existingContacts.length} contacts, skipping API call`,
          );
        ...
        const contactsNeeded =
          MAX_CONTACTS_PER_COMPANY - existingContacts.length;
        this.logger.log(
          `Company ${company.name} has ${existingContacts.length} contacts, need ${contactsNeeded} more`,
        );
        ...
        while (retries < maxRetries) {
          try {
            ...
            if (
              ...
              if (retries < maxRetries) {
                this.logger.warn(
                  `Rate limit hit for ${company.name}, retrying in ${backoffDelay}ms (attempt ${retries}/${maxRetries})`,
                );
                await new Promise((resolve) =>
                  setTimeout(resolve, backoffDelay),
                );
            ...
            this.logger.error(
              `Failed to find people for ${company.name} after ${retries} retries:`,
              error,
            );
            return {
              company,
      ...
      for (const { company, people, existingCount } of batchResults) {
        ...
        for (const personData of sortedPeople) {
          ...
          if (contactsSavedForCompany >= MAX_CONTACTS_PER_COMPANY) {
            this.logger.log(
              `Reached limit of ${MAX_CONTACTS_PER_COMPANY} contacts for ${
                company.name
              }, skipping remaining ${
                sortedPeople.length - contactsSavedForCompany
              } contacts`,
            );
            break;
          }
          ...
          try {
            ...
            if (!personData.name || !personData.title) {
              this.logger.warn(
                `Skipping person with missing data:`,
                personData,
              );
              continue;
            }
            ...
            if (
              ...
              contactMap.has(personData.linkedinUrl)
            ) {
              this.logger.log(
                `Skipping duplicate contact: ${personData.name} (${personData.linkedinUrl})`,
              );
              continue;
            }

            // Find or create person
            let person = await this.personaService.findByLinkedIn(
              personData.linkedinUrl,
            );
            ...
            if (!existingRole) {
              ...
              this.logger.log(
                `Company-person relationship already exists for ${person.full_name} at ${company.name}`,
              );
            }
            ...
            this.logger.error(
              `Failed to save person ${personData.name}:`,
              error,
            );
          }
        }
      ...
      this.emitSignalProgress(signal.id, {
        ...
        progress: Math.round(
          (((batchIndex + 1) * PEOPLE_BATCH_SIZE) / savedCompanies.length) *
            100,
        ),
      });
    }
    ...
    this.logger.log(
      `✅ PEOPLE SAVED: Successfully saved ${savedContacts.length} contacts from ${savedCompanies.length} companies!`,
    );
    this.logger.log(
      `🎉 SIGNAL EXECUTION COMPLETE: Found ${savedEvents.length} events → ${savedCompanies.length} companies → ${savedContacts.length} contacts`,
    );
  ...
  private async executeFundingSignal(): Promise<any> {
  ...
  private async executeHiringSignal(signal: Signal): Promise<any> {
    const { configuration } = signal;
    ...
    return {
      ...
      stats: {
        eventsDiscovered: 0,
        companiesFound: 0,
        contactsDiscovered: (agentResult.result as any[])?.length || 0,
        messagesGenerated: 0,
        messagesSent: 0,
      },
    };
  ...
  async discoverSignal(dto: DiscoverSignalDto): Promise<any> {
    ...
    await this.promptsService.getPublishedPromptBody('signal-discovery');
    ...
    if (dto.preview) {
      ...
      return {
        ...
        preview_results: await this.previewSignalResults(),
      };
    }
  ...
  private parseEventResults(rawResult: any): any[] {
    try {
      this.logger.log(
        `Agent returned ${
          Array.isArray(rawResult) ? rawResult.length : 'non-array'
        } result(s)`,
      );
      ...
      if (Array.isArray(rawResult)) {
        this.logger.log(
          `Using structured event array: ${rawResult.length} events`,
        );
      ...
      this.logger.warn(
        'Expected array from event discovery agent, got:',
        typeof rawResult,
      );
      return [];
    } catch (error) {
  ...
  private parseSponsorResults(rawResult: any): any[] {
    try {
      ...
      if (Array.isArray(rawResult)) {
        this.logger.log(
          `Using structured sponsor array: ${rawResult.length} companies`,
        );
      ...
      this.logger.warn(
        'Expected array from sponsor extraction agent, got:',
        typeof rawResult,
      );
      return [];
    } catch (error) {
  ...
  private parseContactResults(rawResult: any): any[] {
    try {
      ...
      if (Array.isArray(rawResult)) {
        this.logger.log(
          `Using structured contacts array: ${rawResult.length} people`,
        );
      ...
      this.logger.warn(
        'Expected array from people enricher agent, got:',
        typeof rawResult,
      );
      return [];
    } catch (error) {
  ...
  private async parseNaturalLanguageSignal(prompt: string): Promise<any> {
    ...
    const type =
      keywords.find((k) => prompt.toLowerCase().includes(k)) || 'conference';
  ...
  private async previewSignalResults(): Promise<any> {
  ...
  async getSignalExecutions(
    signalId: string,
    page: number = 1,
    limit: number = 10,
  ): Promise<{ data: SignalExecution[]; pagination?: any }> {
  ...
  private getRoleCategory(title: string): string {
    ...
    if (titleLower.includes('sales') || titleLower.includes('revenue'))
      return 'sales';
    if (titleLower.includes('partner') || titleLower.includes('alliance'))
      return 'partnerships';
    if (
